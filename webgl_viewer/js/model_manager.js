...
import { deleteAssetCacheEntry, fetchArrayBuffer, fetchJSON } from './asset_fetcher.js';
...
export class ModelManager {
    constructor(gl) {
        ...
        this._nonRenderableHashes = new Set();
        ...
        // Demo mesh packs collapse hundreds of small HTTP requests into a few
        // content-addressed bundle fetches. Entries are released after warmup so
        // the source ArrayBuffers do not remain beside their uploaded GPU data.
        this._meshPackLoads = new Map(); // pack path -> { promise, releaseTimer }
    ...
    async _loadDemoMeshPackSlice(ref, { usePersistentCache = true } = {}) {
        const path = `assets/demo/${ref.packFile}`;
        let cached = this._meshPackLoads.get(path);
        if (!cached) {
            cached = {
                promise: fetchArrayBuffer(path, { usePersistentCache: !!usePersistentCache }),
                releaseTimer: null,
            };
        ...
        cached.releaseTimer = setTimeout(() => {
            const live = this._meshPackLoads.get(path);
            if (live === cached) this._meshPackLoads.delete(path);
        }, 20000);
    ...
    getLodSubmeshes(hash, lod = 'high') {
        ...
        if (!file) return [];
        return [{ file, material: entryMat, skinned: false, boneIds: [], hasBlendWeights: false, hasBlendIndices: false }];
    }
    ...
    async loadMeshFile(file, {
        ...
        try {
            buf = packedRef
                ...
                : await fetchArrayBuffer(path, { usePersistentCache: !!usePersistentCache });
        } catch (e) {
    ...
    _createPlaceholderMesh() {
        ...
        const lineIndices = createWireframeIndices(indices, Uint32Array);
    ...
    _parseAndUploadMesh(key, arrayBuffer) {
        ...
        if (magic !== 'MSH0' || (version < 1 || version > 9)) {
        ...
        const positions = packed ? new Uint16Array(arrayBuffer, headerBytes, vertexCount * 3) : new Float32Array(arrayBuffer, headerBytes, vertexCount * 3);
        ...
        gl.enableVertexAttribArray(0);
        gl.vertexAttribPointer(0, 3, packed ? gl.HALF_FLOAT : gl.FLOAT, false, 0, 0);
        ...
        const retainPick = !packed
            && !!this.retainCpuPickData
            && vertexCount > 0
            && indexCount > 0
            && indexCount <= 300000;
        const pickPositions = retainPick ? new Float32Array(positions) : null;
        ...
        return {
            ...
            pickPositions,
            pickIndices,
            approxBytes: (posBytes + nrmBytes + uvBytes + uv1Bytes + uv2Bytes + tanBytes + col0Bytes + col1Bytes + blendWeightsBytes + blendIndicesBytes + indices.byteLength + lineIdxBytes + pickBytes),
            bounds,
            radius,
