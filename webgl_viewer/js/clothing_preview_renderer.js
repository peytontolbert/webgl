    ...
    _render(time) {
        ...
        if (this.drawables.length) {
            ...
            for (const row of this.drawables) {
                ...
                // ModelManager uploads 16-bit indices whenever a mesh fits in that
                // range. The draw type must match the element buffer exactly.
                gl.drawElements(gl.TRIANGLES, row.mesh.indexCount, row.mesh.indexType, 0);
            }
