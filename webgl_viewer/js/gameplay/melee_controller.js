...
export class MeleeController {
    ...
    canUse() {
        return !!this.app?.ped
            ...
            && this.lifeState === 'alive'
            && !this.app?.phoneController?.active
    ...
    applyAuthoritativeState({ health, armor, dead = false, respawn = false } = {}) {
        ...
        if (respawn) {
            this._respawnPlayer();
        } else if (dead || this.playerHealth <= 0) {
            if (this.lifeState !== 'dead') this._setLifeState('dead', PLAYER_DEATH_RESPAWN_SECONDS, 'melee_death_a');
        } else if (this.lifeState === 'dead') {
            // Character activation and reconnect messages carry the repaired
            // profile but are not respawn events. Clear stale local death state
            // without moving the player or interrupting non-death knockdowns.
            this._clearLifeState();
        }
        this._uiVersion++;
    }
