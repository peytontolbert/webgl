...
const NEXUS_INVENTORY_IMAGE_ROOT = 'assets/nexus_inventory/images/';
...
export class InventoryOverlay {
    ...
    _bindEvents() {
        ...
        this.dialog.addEventListener('keydown', (event) => {
            if (event.repeat || (event.key !== 'Escape' && event.key !== 'Tab')) return;
            event.preventDefault();
            event.stopPropagation();
            this.close();
        });
        this.dialog.addEventListener('click', (event) => {
            const clickedPanel = event.target?.closest?.('.nx-inventory-panel');
            const clickedContextMenu = event.target?.closest?.('#inventoryContextMenu');
            if (event.target === this.dialog || (!clickedPanel && !clickedContextMenu)) {
                this.close();
                return;
            }
    ...
    _getItems() {
        ...
        const profileInventory = this.app.multiplayer?.profile?.inventory || {};
        const items = new Map();
        if (inventory?.weapon) {
        ...
        const cocaLeaves = Math.max(0, Number(profileInventory.coca_leaves) || 0);
        if (cocaLeaves > 0) {
            items.set(ITEM_DEFINITIONS.cocaLeaves.slot, {
                ...ITEM_DEFINITIONS.cocaLeaves,
                quantity: cocaLeaves,
                durability: 100,
                canUse: false,
            });
        }
        const phone = this.app.phoneController;
        items.set(ITEM_DEFINITIONS.phone.slot, {
            ...ITEM_DEFINITIONS.phone,
            quantity: 1,
            durability: 100,
            active: !!phone?.active,
            canUse: !!phone?.active || !!phone?.canUse?.(),
        });
        return items;
    }
    ...
    _inventoryKey(items) {
        return Array.from(items.entries()).map(([slot, item]) => [
            ...
            item.magazineAmmo, item.reserveAmmo, item.canUse, item.active,
        ].join(':')).join('|');
    }
    ...
    _syncDetails(item) {
        ...
        if (this.detailMeta) {
            if (item.type === 'weapon') {
                ...
                this.detailMeta.textContent = 'Harvesting material';
            } else if (item.type === 'phone') {
                this.detailMeta.textContent = item.active ? 'In use' : 'Ready';
            } else {
                this.detailMeta.textContent = item.canUse ? 'Ready to install' : 'Draw Glock-17 to install';
            }
        }
        if (this.detailAction) {
            this.detailAction.disabled = item.type === 'material' || (item.type === 'component' && !item.canUse) || (item.type === 'phone' && !item.canUse);
            this.detailAction.textContent = item.type === 'weapon'
                ? (item.equipped ? 'Holster' : 'Equip')
                : (item.type === 'ammo' ? 'Reload' : (item.type === 'material' ? 'Stored' : (item.type === 'phone' ? (item.active ? 'Put away' : 'Use') : 'Install')));
        }
    }
    ...
    _showContextMenu(x, y, item) {
        ...
        if (action) {
            action.disabled = item.type === 'material' || (item.type === 'phone' && !item.canUse);
            action.textContent = item.type === 'weapon'
                ? (item.equipped ? 'Holster' : 'Equip')
                : (item.type === 'ammo' ? 'Reload' : (item.type === 'material' ? 'Stored' : (item.type === 'phone' ? (item.active ? 'Put away' : 'Use') : 'Install')));
        }
    ...
    useSlot(slot) {
        ...
        if (item.type === 'weapon') {
            ...
            changed = !!this.app.phoneController?.toggle?.();
            this._setStatus(changed ? (this.app.phoneController?.active ? 'Phone in use' : 'Phone put away') : 'Phone unavailable');
            if (changed) this.close();
        }
