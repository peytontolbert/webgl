...
function itemCard(item) {
    ...
    const garment = document.createElement('span'); garment.className = 'garment'; garment.textContent = item.component.toUpperCase();
...
async function load() {
    const [response, convertedResponse] = await Promise.all([
        fetch('/assets/clothingpack5m_catalog.json?v=20260814-auto-preview', { cache: 'no-store' }),
    ...
    for (const component of components) { const option = document.createElement('option'); option.value = component; option.textContent = component.toUpperCase(); byId('component').append(option); }
    try {
