...
export class App {
    ...
    async loadTextures() {
        try {
            ...
            await this.terrainRenderer.loadTexture('normal', 'assets/normalmap.webp');
            ...
            if (tt && typeof tt === 'object') {
                ...
                for (const k of kinds) {
                    ...
                    await this.terrainRenderer.loadTexture(`${k}.diffuse`, `assets/textures/${base}_diffuse.webp`);
                    if (entry.has_normal) {
                        await this.terrainRenderer.loadTexture(`${k}.normal`, `assets/textures/${base}_normal.webp`);
                    }
                }
                ...
                if (grassBase) {
                    await this.terrainRenderer.loadTexture('diffuse', `assets/textures/${grassBase}_diffuse.webp`);
                    if (tt.grass?.has_normal) {
                        await this.terrainRenderer.loadTexture('normal', `assets/textures/${grassBase}_normal.webp`);
                    }
                }
            }
            ...
            if (mainTexture) {
                ...
                await this.terrainRenderer.loadTexture('diffuse', `assets/textures/${name}_diffuse.webp`);
                // Load normal map if available
                if (tex.has_normal) {
                    await this.terrainRenderer.loadTexture('normal', `assets/textures/${name}_normal.webp`);
                }
            }
            ...
            if (info.texture_info.layers) {
                for (let i = 0; i < Math.min(4, info.texture_info.layers.length); i++) {
                    ...
                    await this.terrainRenderer.loadTexture(`layer${i + 1}`, `assets/textures/${layer.name}_diffuse.webp`);
                    ...
                    const tryNormal = async (key, baseName, { preferAltFirst = false } = {}) => {
                        const canonical = `assets/textures/${baseName}_normal.webp`;
                        const alt = `assets/textures/${baseName}_nm_diffuse.webp`;
            ...
            if (info.texture_info.blend_mask) {
                await this.terrainRenderer.loadTexture('blendMask', `assets/textures/terrain_blend_mask.webp`);
            }
