#!/usr/bin/env python3
"""Build compact runtime data for the fixed 150 x 150 m /demo district.
...
def _pack_default_lod_meshes(
    ...
    """Pack the runtime-resolved medium LOD into a few shared transport files.

    The MSH0 payloads remain byte-for-byte identical. Only their manifest paths
    become range references understood by ModelManager, eliminating hundreds of
    individual requests without changing geometry or material ownership.
    """
    ...
    for rel in sorted(priorities, key=lambda item: (priorities[item], item)):
        ...
        try:
            ...
            continue
        if payload:
            source_files.append((rel, payload))
    ...
    for rel, payload in source_files:
    ...
    return {
        ...
        "sourceBytes": sum(len(data) for _rel, data in source_files),
