...
// Regression: melee targeting must query the shared NPC spatial grid using
// the attacking ped origin, never vehicle-sweep locals.
const targetingSystem = new NpcSystem({ ped: { posData: [0, 0, 1.2] } }, null);
targetingSystem.npcs.push({
  id: 'melee_target', state: 'wander', role: 'civilian',
  x: 1.0, y: 0, feetZ: 0, health: 100,
});
targetingSystem._rebuildSpatialGrid();
const meleeTarget = targetingSystem.findMeleeTarget({
  origin: [0, 0, 1.2],
  heading: 0,
  maxDistance: 2.3,
  coneDot: 0.1,
});
assert.equal(meleeTarget?.id, 'melee_target');
