...
def main() -> int:
    ...
    while True:
        try:
            ...
            run(["ssh", args.remote, f"mkdir -p '{args.demo_root}/dist/assets/custom_clothing' '{args.demo_root}/dist/assets/models/custom_clothing/clothingpack5m' '{args.demo_root}/dist/assets/models_textures'"], root)
            run(["scp", str(manifest_path), f"{args.remote}:{args.demo_root}/dist/assets/custom_clothing/"], root)
            mesh_files = sorted({
            ...
            if mesh_files:
                run(["scp", *mesh_files, f"{args.remote}:{args.demo_root}/dist/assets/models/custom_clothing/clothingpack5m/"], root)
            if texture_paths:
                run(["scp", *[str(root / "assets" / value) for value in texture_paths], f"{args.remote}:{args.demo_root}/dist/assets/models_textures/"], root)
            last_digest = digest
