...
def _material_for_shader(shader: Any, textures: dict[str, tuple[np.ndarray | None, str | None, str]]) -> dict[str, Any]:
    # This exporter resolves every texture that is actually present in the
    # resource YTD. Raw shader parameters may still name external dictionaries;
    # the browser must not turn those debug values into unverified asset URLs.
    material: dict[str, Any] = {"textureBindingsExplicit": True}
